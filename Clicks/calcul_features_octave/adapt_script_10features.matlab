...
%%% GLOBAL PARAMETERS
% paths
save_features = "~/Data/DOLPHINFREE/Scripts/Clicks/calcul_features_octave/features/";

% high pass filter
fcut=50000;    %cutting freq
...
%for i=1:10
    ...
    printf("\rProcessing file %d of %d.", i, length(NomFich));

% ouverture fichier son + extraction parametres 
   nomfichierwav=NomFich{i};

   Chemin = '/media/loic/DOLPHINFREE/Acoustique';
   Annee = ['/' nomfichierwav(5:8) '/'];
   fichierwav = [Chemin Annee nomfichierwav];
   [s,fm] = audioread(fichierwav,[1 10]);
   clear s;
   Tm=1/fm;

% ouverture fichier detection associé au fichier son ci-dessus
   Chemin = "/home/loic/Data/DOLPHINFREE/Scripts/Clicks/Results/peaks_new_method_02052022_txt/";
   nomfichierdetect=[substr(nomfichierwav, -27, -4),'_peaks.txt'];
   fichierdetect = [Chemin nomfichierdetect];

   [date_detection_clic] = textread(fichierdetect) ;
   date_detection_clic = date_detection_clic*Tm;
...
% test pour savoir si on est dans la même série
   ...
   if i==1
      ...
      % Si les heures de deux fichier ne se suivent pas alors
      % Ce sont des séries différentes
%{
      jour_brut=str2num((substr(NomFich{i}, -13, -11)))+str2num((substr(NomFich{i}, -19, -15)));
      heure_brut=str2num((substr(NomFich{i}, -10, -8)));
      min_brut=str2num((substr(NomFich{i}, -8, -6)));
      jour_prev=str2num((substr(NomFich{i-1}, -13, -11)))+str2num((substr(NomFich{i-1}, -19, -15)));
      heure_prev=str2num((substr(NomFich{i-1}, -10, -8)));
      min_prev=str2num((substr(NomFich{i-1}, -8, -6)));
      datemin=jour_brut+heure_brut*60+min_brut;
      datemin_prev=jour_prev+heure_prev*60+min_prev;

      if abs(datemin-datemin_prev)>1
...
%}
      ...
      % Autre option : changer à chaque audio...
...
% ouverture fichier son click
      ...
      extrait=audioread(fichierwav,[inicioN finN]);
      senal = extrait(:,1);
...
% calcul Delta t -10dB
       ...
       valplus=valmax;
       incrplus=0;
       while valplus>valmax/sqrt(10) && (indicemax+incrplus<length(enveloppe))    % Delta t -10dB
            incrplus=incrplus+1;
       ...
       valmoins=valmax;
       incrmoins=0;
       while valmoins>valmax/sqrt(10) && (indicemax+incrmoins>1)   % Delta t -10 dB
...
% calcul Delta t -20dB
       ...
       valplus=valmax;
       incrplus=0;
       while (valplus>valmax/10) && (indicemax+incrplus<length(enveloppe))   % Delta t -20dB
            incrplus=incrplus+1;
            valplus=enveloppe(indicemax+incrplus);
       endwhile  
       ...
       valmoins=valmax;
       incrmoins=0;
       while (valmoins>valmax/10) && (indicemax+incrmoins>1)   % Delta t -20dB
            incrmoins=incrmoins-1;
            valmoins=enveloppe(indicemax+incrmoins);
       endwhile
...
% Delta f -3bB
       ...
       energieplus=valmax;
       incrplus=0;
       while (energieplus>valmax/sqrt(2)) && (indicemax+100+incrplus<255) % Delta F -3dB
            incrplus=incrplus+1;
       ...
       energiemoins=valmax;
       incrmoins=0;
       while (energiemoins>valmax/sqrt(2)) && (indicemax+100+incrmoins>1)  % Delta F -3dB
            incrmoins=incrmoins-1;
...
% Delta f -10bB
       ...
       energieplus=valmax;
       incrplus=0;
       while (energieplus>valmax/sqrt(10)) && (indicemax+100+incrplus<255)  % Delta F -10dB
            incrplus=incrplus+1;
            energieplus=abs(zoom_Tfourier(indicemax+100+incrplus));
       endwhile  
       ...
       energiemoins=valmax;
       incrmoins=0;
       while (energiemoins>valmax/sqrt(10)) && (indicemax+100+incrmoins>1)  % Delta F -10dB
            incrmoins=incrmoins-1;
            energiemoins=abs(zoom_Tfourier(indicemax+100+incrmoins));
       endwhile
...
%   ylabel('number of clicks','fontweight','bold','fontsize',16)
   ...
   titre=[save_features,'histogram-',substr(NomFich{i},-27,-4),'.jpg'];
   print(titre,'-djpg');
...
dlmwrite([save_features, "features-", substr(NomFich{i},-27,-4), ".csv"],
    reshape([frequencepeak'; frequencecentr'; DeltaF_3dB'; Deltat_10dB'; DeltaF_10dB';
        Deltat_20dB'; DeltaF_rms'; Deltat_rms'; [ICI(1) ICI]'; SNR'], 
            length(SNR), 10))
